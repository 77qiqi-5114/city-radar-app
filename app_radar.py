...
with tab1:
    ...
    with m_col:
        ...
        if not df_map.empty:
            m = folium.Map(location=[sel['lat'], sel['lng']], zoom_start=11)
            marker_cluster = MarkerCluster().add_to(m)
            for _, r in df_map.iterrows():
                ind = industry_dict.get(str(r.get('行业门类', '')).upper(), "其他")
                folium.CircleMarker([r['lat'], r['lng']], radius=5, color='#1abc9c', fill=True, popup=f"企业:{str(r.get('企业名称',''))[:8]}<br>行业:{ind}").add_to(marker_cluster)
...
with tab3:
    ...
    if not df_imp.empty:
        ...
        df_imp = df_imp.sort_values('importance', ascending=True) # 从小到大排，出来的条形图就是从下往上增长
        ...
        fig_imp = px.bar(
            ...
            orientation='h', 
            color="importance", 
            color_continuous_scale="Reds" # 还原截图中的深红色调
        )
        # 调整布局让它更像你的截图
        fig_imp.update_layout(height=500, margin=dict(l=20, r=20, t=20, b=20))
        st.plotly_chart(fig_imp, use_container_width=True)
    else:
        st.warning("💡 数据库中未找到预警因子权重数据。")
