...
def init_connection():
    try:
        ...
        st.error(f"🚨 数据库连接失败，请检查 Secrets 配置。错误详情: {e}")
        st.stop()

engine = init_connection()

@st.cache_data(ttl=600) # 缓存10分钟，避免重复查询
def load_data(query):
    try:
...
# ==========================================
with st.sidebar:
    st.title("📡 City-Radar v5.4")
    st.success("✨ 硬件加速引擎已启用") 
...
with tab1:
    ...
    sel = city_map_cfg[city_choice]
    
    m_col, i_col = st.columns([2.5, 1])
    
    with m_col:
        st.subheader(f"🗺️ {city_choice}·产业集聚分布图 (5000点满血版)")
        # 保持 5000 条数据
        ...
        if not df_map.empty:
            ...
            df_map['企业简称'] = df_map['企业名称'].astype(str).apply(lambda x: x[:10] + "..." if len(x) > 10 else x)
            
            # 使用 Plotly Mapbox 实现硬件加速渲染
...
with tab2:
    ...
    if not df_sal_all.empty:
        ...
        with c2:
            if not df_sal.empty:
                fig_sal = px.bar(df_sal, x="premium_rate", y="特征", orientation='h', color="premium_rate", color_continuous_scale="GnBu")
...
with tab3:
    ...
    if not df_risk.empty:
        fig_risk = px.scatter(df_risk, x="count", y="risk", size="count", color="行业板块", text="行业板块", size_max=60)
